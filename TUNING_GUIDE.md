...
| 표시 | 의미 |
|---|---|
| `CLEAR` (초록) | 현재 속도에서 안전한 전환 조향이 있음. TTC 감속 없음 |
...
- `STOP`이 많아서 느림: TTC가 실제로 짧거나 odom, 현재 조향, 차량 형상 설정이 잘못됨
...
검사 horizon 안에 충돌점이 하나라도 있으면 전체 조향 범위에서 회피를 즉시 탐색합니다. horizon 끝까지 충돌 없는 회피 조향을 찾으면 감속하지 않고, 모든 조향 후보가 위험할 때만 `BRAKE` 또는 `STOP`으로 전환됩니다.
...
너무 높이면 유지한 쪽이 막혔을 때 반대쪽 전환이 늦어질 수 있습니다.
...
```yaml
speed_factor: 0.24
minimum_non_emergency_speed: 1.0
narrow_gap_max_speed: 1.5
max_speed: 3.0
```

일반 속도를 조금 올리려면 `speed_factor`를 `0.24 → 0.27`처럼 `0.02~0.03` 단위로 올립니다.

안전거리가 짧아도 비상이 아닐 때 속도를 높이려면 `minimum_non_emergency_speed`를 `1.0 → 1.1 m/s`로 조정합니다.

기본 차량 반폭으로 재탐색한 좁은 gap에서만 느리면 `narrow_gap_max_speed`를 `1.5 → 1.7 m/s`로 조정합니다.

### 디버그가 `BRAKE`인 경우

horizon 끝까지 충돌 없는 전환 궤적 후보가 없다는 의미입니다. 실제로 옆 공간이 충분히 안전한 것이 확인됐다면 다음 순서로 완화합니다.

1. `avoidance_steering_step: 2.0 → 1.0`
...
```yaml
minimum_crawl_speed: 0.3
```

차량이 너무 느려 조향 자체가 불가능한 경우에만 `0.3 → 0.4 m/s`를 검토합니다.
...
path_rejoin_weight: 2.0    # 2.5까지 증가 검토
```

path에서 벗어난 뒤 복귀가 너무 늦으면 `path_rejoin_distance: 0.5 → 0.4`로 내립니다. 더 작은 이탈에서도 rejoin weight가 적용됩니다.
...
path_rejoin_weight: 2.0         # 1.5까지 감소 검토
...
```yaml
gap_safety_margin: 0.1  # 0.15까지 증가 검토
```
...
```text
curvature_gain(v) = clamp(1 / (0.71 + 0.59 × v²), 0.60, 1.25)
```

이 값은 workspace의 `0818`, `0819_1`, `test_12` bag에서 조향·속도가 안정된 구간을 추출해 얻은 값입니다. `0.35~0.6 / 0.6~0.9 / 0.9~1.2 m/s`에서 실측 gain 중앙값은 각각 약 `1.22 / 0.98 / 0.65`였습니다. 새 타이어, 조향 링크, wheelbase 또는 속도계 보정이 바뀌지 않았다면 네 개의 `curvature_*` 값을 감각적으로 조정하지 않습니다.

가속이 너무 느리면 `max_acceleration: 6.0 → 7.0`처럼 올릴 수 있습니다.
...
### LiDAR가 튀거나 odom 속도가 순간적으로 비정상일 때
...
- LiDAR median filter: 5개 ray
- odom 속도 median: `2 × 5 + 1 = 11`개 샘플

값을 올리면 노이즈에는 강해지지만 작은 장애물과 급격한 속도 변화를 늦게 반영할 수 있습니다. 일반적으로 `5`를 유지하는 것이 좋습니다.
...
```yaml
avoidance_steering_step: 2.0  # 3.0까지 증가
...
curvature_model_speed_squared_gain: 0.59
...
1. 한 번에 한 파라미터만 변경합니다.
2. 거리값은 `0.05 m`, 시간은 `0.02 s`, weight는 `0.1~0.2` 단위로 변경합니다.
3. `test_13`의 50초와 110초 위험구간을 확인합니다.
4. 전체가 아니라 `gap_obs` 선택 구간도 별도로 집계합니다.
5. 큰 좌우 반전이 다시 생기지 않는지 확인합니다.
6. 저속 비율과 충돌 안전성을 함께 비교합니다.
